...
describe("cmd", function() {
  describe("should download and install", function() {
    ...
    verify("0.50/extended", { HUGO_VERSION: "0.50.0/extended" });
  });
});
...
function verify(version, cliEnv = {}) {
  it(version + ", env=" + inspect(cliEnv), function() {
    ...
    exec("npm", ["install", `hugo-cli@${wd}`], {
      cwd
    });

    // then
    // version should be installed
    var result = exec("node_modules/.bin/hugo", ["version"], {
      cwd,
      env: cliEnv
    });
    ...
    assert.ok(
      stdout.indexOf(`Hugo Static Site Generator v${version} `) !== -1,
      `hugo reports version v${version}`
    );
  });
}
