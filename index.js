...
var HUGO_BASE_URL = "https://github.com/gohugoio/hugo/releases/download",
  ...
  HUGO_DEFAULT_VERSION = process.env.HUGO_VERSION || "0.50.0";
...
function withHugo(options, callback) {
  ...
  if (semver.lt(compatVersion, HUGO_MIN_VERSION)) {
    logError(
      "hugo-cli@%s is compatible with hugo >= %s only.",
      cliVersion,
      HUGO_MIN_VERSION
    );
  ...
  compatVersion = compatVersion.endsWith(".0")
    ? compatVersion.slice(0, -2)
    : compatVersion;

  var pwd = __dirname;

  var installDetails = getDetails(
    (extended ? "extended_" : "") + compatVersion,
    TARGET
  );
  ...
  verbose &&
    logDebug("downloading archive from <%s>", installDetails.downloadLink);
  ...
  download(installDetails.downloadLink, archivePath, function(err) {
    ...
    extract(archivePath, extractPath, installDetails).then(
      function() {
